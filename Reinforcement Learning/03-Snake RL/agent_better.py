...
from DQN import DeepQNetwork, QTrainer
...
class Agent:
    ...
    def get_state(self, game): # Get the state defining the environment
        ...
        # Last 3: Warning of 2 steps ahead collision
        ...
        # Second-most immediate point around the SNAKE's HEAD
        point_l2 = Point(head.x - 2*BLOCK_SIZE, head.y)
        point_r2 = Point(head.x + 2*BLOCK_SIZE, head.y)
        point_u2 = Point(head.x, head.y - 2*BLOCK_SIZE) # y axis is positive downwards
        ...
        state = [
            # Warning Straight (instead of danger straight is True)
            # Pure warning WITHOUT Danger doesn't work
            ((dir_r and game.is_collision(point_r2)) or
            (dir_d and game.is_collision(point_d2)) or
            (dir_l and game.is_collision(point_l2)) or
            (dir_u and game.is_collision(point_u2))) and
            ...
            (dir_l and game.is_collision(point_u2))) and
            ...
            (dir_r and game.is_collision(point_u2))) and
    ...
    while True:
        ...
        if game_over:
            ...
            if score > record: # New high score
                record = score
                agent.model.save(filename='RL_Snake2.pth')
